...
int main(int argc, char *argv[])
{
    ...
    uint16_t rgstrInfo[15]; 
 
    while(1){
        rc = modbus_read_registers(ctx, UT_REGISTERS_ADDRESS, 1, rgstrInfo);
        //Change UT_REGISTERS_ADDRESS to the starting register
        //Change 1 to the amount of registers to read

        ASSERT_TRUE(rc == 1, "FAILED (nb points %d)\n", rc);

        //Comment or uncomment as needed to see what's in rgstrInfo
        for(int i = 0; i < 15; i++){
            printf("%u\n", rgstrInfo[i]);
        }

        //Each index in rgstrInfo is a register
    }
